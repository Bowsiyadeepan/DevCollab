...
import { ArrowLeft, Plus, Loader2, Sparkles, X, RotateCcw, CheckCircle2 } from 'lucide-react';
import api from '../utils/api';
import { io } from 'socket.io-client';

// Initialize socket outside component to maintain a single connection
const SOCKET_SERVER = process.env.REACT_APP_API_BASE_URL || 'http://localhost:5001';
...
export default function Board() {
  ...
  const addAllAITasks = async () => {
    ...
    try {
      ...
      setAiError('Failed to add some tasks');
    } finally {
        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 transition-all px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-purple-900/20 active:scale-95"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 transition-all px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-blue-900/20 active:scale-95"
            >
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-2xl p-8 w-full max-w-lg shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            {aiTasks.length === 0 ? (
              <div className="space-y-6">
                <p className="text-gray-400 text-sm">Describe a feature and AI will break it down into development tasks.</p>
                
                <textarea
                  value={aiFeature}
                  onChange={e => setAiFeature(e.target.value)}
                  className="w-full bg-gray-900 border border-gray-700 text-white px-4 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all resize-none placeholder:text-gray-600"
                  placeholder="Example: Build user authentication with login, register and JWT tokens"
                  rows={4}
                />

                <div className="flex flex-wrap gap-2">
                  {["Build login system", "Create dashboard page", "Setup payment integration"].map(example => (
                    <button
                      key={example}
                      onClick={() => setAiFeature(example)}
                ...
                <button
                  ...
                  disabled={aiLoading || !aiFeature.trim()}
                  className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-700 text-white py-4 rounded-xl font-bold transition-all shadow-lg shadow-purple-900/20 flex items-center justify-center gap-2"
                >
                  {aiLoading ? (
                    <>
              ...
              <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-300">
                <div className="flex items-center justify-between">
                  <h3 className="text-green-400 font-bold flex items-center gap-2">
                    ...
                    Generated {aiTasks.length} tasks
                  </h3>
                </div>

                <div className="max-h-64 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                  {aiTasks.map((task, idx) => (
                    <div key={idx} className="bg-gray-900 border border-gray-700 p-4 rounded-xl">
                      <div className="flex justify-between items-start gap-3 mb-2">
                        <h4 className="font-bold text-sm text-white">{task.title}</h4>
                        <span className={`text-[10px] px-2 py-0.5 rounded-md border font-bold uppercase ${getPriorityStyles(task.priority)}`}>
                          {task.priority}
                        </span>
                      </div>
                ...
                <div className="flex gap-4">
                  <button
                    onClick={() => setAiTasks([])}
                    className="flex-1 bg-gray-700 hover:bg-gray-600 text-white py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-gray-700 text-white py-4 rounded-xl font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-green-900/20"
                  >
                    {addingTasks ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" />
